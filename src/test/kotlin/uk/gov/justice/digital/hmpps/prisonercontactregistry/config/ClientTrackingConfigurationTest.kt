...
class ClientTrackingConfigurationTest {
  ...
  fun shouldAddClientIdAndUserNameToInsightTelemetry() {
    // Given
    val token = jwtAuthHelper.createJwtAccessToken(clientId = "prisoner-contact-registry-client", username = "bob")
  ...
  fun shouldAddOnlyClientIdIfUsernameNullToInsightTelemetry() {
    // Given
    val token = jwtAuthHelper.createJwtAccessToken(clientId = "prisoner-contact-registry-client")
    val req = MockHttpServletRequest()
    req.addHeader(HttpHeaders.AUTHORIZATION, "Bearer $token")
    val res = MockHttpServletResponse()
